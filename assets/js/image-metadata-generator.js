jQuery(document).ready(function ($) {
              ...
              if (response.success) {
                ...
                if (metadata.alt_text) {
                  resultsHtml +=
                    ...
                    metadata.alt_text +
                    ' <button class="keep-metadata" data-key="alt_text">Keep</button></li>';
                }
                if (metadata.title) {
                  resultsHtml +=
                    "<li><strong>Title:</strong> " +
                    metadata.title +
                    ' <button class="keep-metadata" data-key="title">Keep</button></li>';
                }
                if (metadata.file_name) {
                  resultsHtml +=
                    "<li><strong>File Name:</strong> " +
                    metadata.file_name +
                    ' <button class="keep-metadata" data-key="file_name">Keep</button></li>';
                }
                if (metadata.caption) {
                  resultsHtml +=
                    "<li><strong>Caption:</strong> " +
                    metadata.caption +
                    ' <button class="keep-metadata" data-key="caption">Keep</button></li>';
                }
                resultsHtml += "</ul>";
  ...
  $("#generate-metadata-form input")
    .not("#all-metadata")
    .click(function () {
      var totalCheckboxes = $("#generate-metadata-form input").not(
        "#all-metadata"
      ).length;
      var checkedCheckboxes = $("#generate-metadata-form input:checked").not(
        "#all-metadata"
      ).length;
